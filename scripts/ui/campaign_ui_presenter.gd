...
func build_fallback_campaign_state(save_data: Dictionary) -> Dictionary:
	...
	return {
		...
		"boss_kind": String(save_data.get("campaign_active_boss_kind", "super" if bool(save_data.get("campaign_in_boss", false)) else "")),
		"pending_boss_kind": String(save_data.get("campaign_pending_boss_kind", "")),
...
func update_summary_labels(
	...
	if wave_boss_button:
		var show_wave_boss_button = should_show_wave_boss_button(campaign_state)
		wave_boss_button.visible = show_wave_boss_button
		wave_boss_button.disabled = not show_wave_boss_button
	if wave_label:
...
func update_map_ui(
	...
	var boss_kind = String(campaign_state.get("boss_kind", ""))
	var pending_boss_kind = String(campaign_state.get("pending_boss_kind", ""))
	var selected_boss_now = bool(campaign_state.get("selected_boss", false))
	var wave_boss_locked_state = pending_boss_kind == "wave" or (in_boss_fight and boss_kind == "wave")
	...
	auto_summary_label.text = "Auto Next Wave: %s | Auto Start Super Boss: %s" % [
	...
	for index in range(wave_buttons.size()):
		...
		button.disabled = not is_unlocked or wave_boss_locked_state
	...
	var boss_available = boss_unlocked or (in_boss_fight and boss_kind == "super")
	boss_button.disabled = not boss_available
	boss_button.text = build_boss_button_text(boss_available, in_boss_fight and boss_kind == "super", selected_boss_now)
	boss_button.set_pressed_no_signal(selected_boss_now)

	var can_start_selection = false if wave_boss_locked_state else (
		boss_available if selected_boss_now else selected_wave_number <= highest_wave
	)
...
func build_campaign_header_text(campaign_state: Dictionary) -> String:
	...
	if bool(campaign_state.get("in_boss_fight", false)):
		var boss_kind = String(campaign_state.get("boss_kind", ""))
		if boss_kind == "wave":
			return "Chapter %d - Wave %d Boss" % [chapter_number, int(campaign_state.get("wave", 1))]
		return "Chapter %d - Super Boss" % chapter_number
...
func build_campaign_status_text(campaign_state: Dictionary) -> String:
	...
	if not status_text.is_empty():
		return status_text
	if bool(campaign_state.get("in_boss_fight", false)):
		var label = "Wave Boss" if String(campaign_state.get("boss_kind", "")) == "wave" else "Super Boss"
		return "%s timer: %.1fs remaining" % [label, float(campaign_state.get("boss_time_remaining", 0.0))]
	if String(campaign_state.get("pending_boss_kind", "")) == "wave":
		return "Wave boss ready. Repeat the wave or tap Boss above."
	if bool(campaign_state.get("boss_unlocked", false)) and not bool(campaign_state.get("auto_start_boss", false)):
		return "Super Boss unlocked. Start it from Map or keep farming Wave 10."
...
func build_map_current_state_text(campaign_state: Dictionary) -> String:
	...
	if bool(campaign_state.get("in_boss_fight", false)):
		var boss_kind = String(campaign_state.get("boss_kind", ""))
		current_text = "Current: Wave %d Boss" % int(campaign_state.get("wave", 1)) if boss_kind == "wave" else "Current: Super Boss"
	elif String(campaign_state.get("pending_boss_kind", "")) == "wave":
		current_text = "Current: Wave %d boss incoming" % int(campaign_state.get("wave", 1))
	elif bool(campaign_state.get("boss_unlocked", false)):
		current_text = "Current: Wave 10 cleared, super boss ready"
	else:
		current_text = "Current: Wave %d/10" % int(campaign_state.get("wave", 1))

	var selected_text = "Selected: Super Boss" if bool(campaign_state.get("selected_boss", false)) else (
...
func should_show_wave_boss_button(campaign_state: Dictionary) -> bool:
	return String(campaign_state.get("pending_boss_kind", "")) == "wave" \
		and not bool(campaign_state.get("in_boss_fight", false))
